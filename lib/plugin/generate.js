...
const fs = require('fs')
const generateModels = require('../generator').generateModels
...
function generate (appc, opts, callback) {
  ...
  async.series([
    ...
    function copyConnectorTest (cb) {
      ...
      var toDir = path.join(config.dir, 'test')
      var to = path.join(toDir, 'connector.js')
      var fromBuf = fs.readFileSync(from, 'utf8')
      var toBuf = _.template(fromBuf, config)
    ...
    function writeOutPackageJSON (cb) {
      ...
      var ignore = ['inquirer', fromPKG.name] // these packages don't need to be copied since they are used by this plugin
