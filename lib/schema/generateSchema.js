...
exports.generateSchema = function generateSchema(next) {
	 ...
	 */
	var confDir = self.tmpdir,
		hashedFilename = 'salesforce-schema-' + self.md5(self.config.username + self.config.token) + '.json';
	...
	function useCachedVersion() {
		...
		fs.readFile(cachedFileName, 'UTF-8', function readFile(err, contents) {
			if (err) {
				next && next(self.parseError(err));
			}
			else {
				next && next(null, JSON.parse(contents));
			}
		});
