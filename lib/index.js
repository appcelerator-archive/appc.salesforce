...
function Connector(config) {
	return Mobware.createConnector(config,{
		...
		fetchSchema: function(next){
			...
			fs.exists(cachedFileName, function(exists){
				if (exists) {
					...
					if (this.config.schemaRefresh && this.config.schemaRefresh >= ago) {
						return fs.readFile(cachedFileName,function(err,buf){
			...
			this.config.schemaRefresh && setTimeout(function(){
				fs.unlink(cachedFileName, function(){
					this.fetchSchema();
				}.bind(this));
			}.bind(this), this.config.schemaRefresh || 60000 * 30);
		},
