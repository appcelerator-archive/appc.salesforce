...
exports.create = function(APIBuilder, server) {
	...
	return Connector.extend({
		...
		connect: function connect(next) {
			...
			this.connection.login(username, password, function connectLoginCallback(err, userInfo) {
				if (err) { return next(err); }
					...
					// Once an hour, refresh the login.
					if (this.refreshLoginIntervalID){
						clearInterval(this.refreshLoginIntervalID);
					}
					// Once an hour, refresh our login.
					this.refreshLoginIntervalID = setInterval(this.refreshLogin.bind(this), 60 * 60 * 1000);
					return next();
				}
			}.bind(this));
		},
		...
		apex: function(verb, path, body, next) {
			...
			this.connection.apex[verb](path, body, next);
		},

		// support search using SOSL
		search: function(sosl, next) {
			this.connection.search(sosl, next);
		},

		// support identity API
		identity: function(next) {
			this.connection.identity(next);
		},

		// support apiUsage API
		apiUsage: function(next){
			next(null, this.connection.limitInfo);
		},
		...
		chatter: function(operation, resource, params, next){
			...
			args.push(next);
			var scope = this.connection.chatter.resource(resource);
		...
		refreshLogin: function refreshLogin() {
			var username = this.config.username,
				password = this.config.password + this.config.token;
			this.connection.login(username, password, function reConnectLoginCallback(err, userInfo) {
				if (err) {
					this.logger.error('login refresh failed');
					this.logger.error(err);
					clearInterval(this.refreshLoginIntervalID);
					this.refreshLoginIntervalID = null;
				}
				else {
					this.logger.info('login refreshed');
				}
			}.bind(this));
		},
		generateModels: function generateModels(schema, next) {
			...
			generator.generateModels(this.name, modelDir, schema, this.config.modelAutogen === undefined ? true : this.config.modelAutogen, function(err){
				if (err) { return next(err); }
